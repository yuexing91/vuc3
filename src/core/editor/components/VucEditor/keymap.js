    ...
    _handleKeyMap(e) {
      ...
      if (e.ctrlKey && !e.altKey && !e.shiftKey) {
        switch (e.keyCode) {
          ...
          // ctrl + X 剪切
          case keyCode.X:
            this.cutNode(curNode);
            return;
          // ctrl + V 粘贴
          case keyCode.V:
            this.parseNode(curNode, 'after');
            return;
          // ctrl + I 插入
          case keyCode.I:
            this.insertToSlot(curNode);
            return;
          // ctrl + P 重复
          case keyCode.P:
            this.repeatNode(curNode);
            return;
          // ctrl + Y 重做
          case keyCode.Y:
            return;
        ...
        switch (e.keyCode) {
          case keyCode.Up:
            this.moveToBefore(curNode);
            return;
          case keyCode.Down:
            this.moveToAfter(curNode);
            return;
          case keyCode.Left:
            this.moveToBefore(curNode);
            return;
        ...
        switch (e.keyCode) {
          ...
          case keyCode.Down:
            if (curNode.children && curNode.children.length) {
              let child = curNode.children.includes(this.__select__parent__child__curNode__)
                ? this.__select__parent__child__curNode__
                : curNode.children[0];
